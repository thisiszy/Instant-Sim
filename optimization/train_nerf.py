...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale', type=float, default=0.8, help="scale camera location into box[-bound, bound]^3")
    ...
    if not opt.workspace:
        opt.workspace = os.path.basename(os.path.normpath(opt.path))
    print(opt)
    ...
    if opt.test:
        ...
        trainer = Trainer(opt.workspace, opt, model, device=device, workspace=opt.workspace, criterion=criterion, fp16=opt.fp16, metrics=metrics, use_checkpoint=opt.ckpt)
