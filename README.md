...
- <code>render_images_grad()</code> should use batchified rays rendering (by turning on <code>stage</code> in <code>self.model.render()</code>).The batch size is hardcoded to 4096 here. This is essential for avoiding OOM (out of memory) error, due to gradient retaining. The result image can be reconstructed by properly concateanting the rendering rgbs.  
